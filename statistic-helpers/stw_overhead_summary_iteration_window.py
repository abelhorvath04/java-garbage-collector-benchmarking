...
# Broad matcher for unified GC pause lines.
# It intentionally only extracts the body after GC(id); STW classification happens later.
...
class BenchmarkDurations:
    ...
    steady_state_start_ns: int
    steady_state_end_ns: int
...
class StwAggregate:
    ...
    min_gc_uptime_s_in_steady_state_window: float | None
    max_gc_uptime_s_in_steady_state_window: float | None
    stw_window_start_ns: int
    stw_window_end_ns: int
...
def overlap_ns(start_a: int, end_a: int, start_b: int, end_b: int) -> int:
...
def normalize_pause_category(category: str) -> str:
    ...
    # ZGC phase lines can contain prefixes such as:
    #   y: Pause Mark Start ...
    #   Y: Pause Mark Start ...
    #   O: Pause Mark End ...
...
def is_strict_stw_pause(tags_raw: str, category: str) -> bool:
    ...
    # G1 and general unified GC summary lines.
    if tags == ["gc"]:
        return True

    # ZGC strict short STW phase lines.
...
def extract_category_and_duration(body: str) -> tuple[str, str, int] | None:
    matches = list(DURATION_RE.finditer(body))
    if not matches:
        return None

    # Use the first duration token after the pause category.
    # This avoids accidentally reading trailing User/Sys/Real values as pause duration.
    duration_match = matches[0]
    duration_ns = parse_duration_to_ns(duration_match.group("value"), duration_match.group("unit"))
    ...
    return raw_category, normalized_category, duration_ns
...
def parse_gc_logs(
    ...
    for path in paths:
        with path.open("r", encoding="utf-8", errors="replace") as f:
            for raw_line in f:
                ...
                extracted = extract_category_and_duration(match.group("body"))
                if extracted is None:
                    continue

                _raw_category, category, duration_ns = extracted

                if not is_strict_stw_pause(match.group("tags"), category):
                    continue

                uptime_s = float(match.group("uptime"))
                pause_end_ns = parse_uptime_to_ns(match.group("uptime"))
                pause_start_ns = max(0, pause_end_ns - duration_ns)

                # Exact steady-state window:
                # starts at the first included iteration start,
                # ends at the last included iteration end.
                stw_overlap_ns = overlap_ns(
                    pause_start_ns,
                    pause_end_ns,
                    stw_window_start_ns,
                    stw_window_end_ns,
                ...
                breakdown_ns[category]["total_ns"] += stw_overlap_ns
                total_count += 1
                total_ns += stw_overlap_ns
    ...
    return StwAggregate(
        ...
        min_gc_uptime_s_in_steady_state_window=min_uptime_s,
        max_gc_uptime_s_in_steady_state_window=max_uptime_s,
        stw_window_start_ns=stw_window_start_ns,
        stw_window_end_ns=stw_window_end_ns,
    )
...
def make_benchmark_durations(durations: list[int], starts: list[int]) -> BenchmarkDurations:
    if not durations or not starts or len(durations) != len(starts):
        raise RuntimeError("No benchmark measurements left after warmup cutoff")
    ...
    return BenchmarkDurations(
        ...
        steady_state_start_ns=min(starts),
        steady_state_end_ns=max(ends),
    )
...
def parse_benchmark_json(path: Path, warmup_cutoff_ns: int) -> BenchmarkDurations:
    ...
    return make_benchmark_durations(durations, starts)
...
def build_summaries(args: argparse.Namespace) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
        ...
        for benchmark in args.benchmark:
            try:
                ...
                stw = parse_gc_logs(
                    ...
                    stw_window_start_ns=durations.steady_state_start_ns,
                    stw_window_end_ns=durations.steady_state_end_ns,
                )
            ...
            throughput_rows.append({
                ...
                "steady_state_start_s": ns_to_s(durations.steady_state_start_ns),
                "steady_state_end_s": ns_to_s(durations.steady_state_end_ns),
                "steady_state_wall_clock_span_ms": ns_to_ms(durations.steady_state_end_ns - durations.steady_state_start_ns),
            ...
            stw_rows.append({
                ...
                "steady_state_end_s": ns_to_s(durations.steady_state_end_ns),
                "steady_state_wall_clock_span_ms": ns_to_ms(durations.steady_state_end_ns - durations.steady_state_start_ns),
                "iteration_total_duration_ms": durations.total_duration_ms,
                "min_gc_uptime_s_in_steady_state_window": stw.min_gc_uptime_s_in_steady_state_window,
                "max_gc_uptime_s_in_steady_state_window": stw.max_gc_uptime_s_in_steady_state_window,
            })
    ...
    validation = {
        ...
        "total_strict_stw_pause_events_in_iteration_window": sum(r["stw_pause_count"] for r in stw_rows),
        "total_stw_ms_in_iteration_window": sum(r["total_stw_ms"] for r in stw_rows),
        "errors": errors,
    }
...
def write_throughput_csv(path: Path, throughput_rows: list[dict[str, Any]]) -> None:
    fields = [
        ...
        "steady_state_end_s",
        "steady_state_wall_clock_span_ms",
        "fastest_environment",
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for row in throughput_rows:
            out = {
                ...
                "steady_state_start_s": f"{row['steady_state_start_s']:.9f}",
                "steady_state_end_s": f"{row['steady_state_end_s']:.9f}",
                "steady_state_wall_clock_span_ms": f"{row['steady_state_wall_clock_span_ms']:.6f}",
...
def write_stw_csv(path: Path, stw_rows: list[dict[str, Any]]) -> None:
    ...
    fields = [
        ...
        "min_gc_uptime_s_in_steady_state_window",
        "max_gc_uptime_s_in_steady_state_window",
    ] + category_fields
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for row in stw_rows:
            out: dict[str, Any] = {
                ...
                "steady_state_end_s": f"{row['steady_state_end_s']:.9f}",
                "steady_state_wall_clock_span_ms": f"{row['steady_state_wall_clock_span_ms']:.6f}",
                "iteration_total_duration_ms": f"{row['iteration_total_duration_ms']:.6f}",
                "min_gc_uptime_s_in_steady_state_window": "" if row["min_gc_uptime_s_in_steady_state_window"] is None else f"{row['min_gc_uptime_s_in_steady_state_window']:.6f}",
                "max_gc_uptime_s_in_steady_state_window": "" if row["max_gc_uptime_s_in_steady_state_window"] is None else f"{row['max_gc_uptime_s_in_steady_state_window']:.6f}",
            }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate Renaissance throughput and strict JVM STW pause overhead in the exact included-iteration time window."
    )
    parser.add_argument(
        "--env",
        action="append",
...
def main() -> int:
    ...
    print(f"  total strict STW pause events in iteration window: {validation['total_strict_stw_pause_events_in_iteration_window']}")
    print(f"  total STW ms in iteration window: {validation['total_stw_ms_in_iteration_window']:.6f}")
